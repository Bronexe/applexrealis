...
// Función simple para subida de archivos optimizada
export async function uploadFileSimple(formData: FormData) {
  ...
  // Para archivos muy grandes (>10MB), usar timeout más largo
  ...
  try {
    ...
    const uploadOptions = {
      ...
      // Para archivos grandes, deshabilitar compresión
      ...(isLargeFile && { 
        transform: null,
...
export async function uploadFileWithRetrySimple(
  formData: FormData,
  maxRetries: number = 3
) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const result = await uploadFileSimple(formData)
