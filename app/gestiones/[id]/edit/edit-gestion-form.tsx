...
import { updateGestion, getUserCondos } from "@/lib/actions/gestiones"
import { useToast } from "@/hooks/use-toast"
import type { Gestion } from "@/lib/actions/gestiones"
...
const ESTADO_OPTIONS = [
  ...
  { value: "pendiente", label: "Pendiente", color: "outline" },
...
interface EditGestionFormProps {
  gestion: Gestion
}

export function EditGestionForm({ gestion }: EditGestionFormProps) {
  const router = useRouter()
  const { toast } = useToast()
  const [isLoading, setIsLoading] = useState(false)
  const [condos, setCondos] = useState<Condo[]>([])
  const [tags, setTags] = useState<string[]>(gestion.tags || [])
  const [newTag, setNewTag] = useState("")
  const [date, setDate] = useState<Date | undefined>(
    gestion.fecha_limite ? new Date(gestion.fecha_limite) : undefined
  )
  ...
  const [formData, setFormData] = useState({
    ...
    condominio_id: gestion.condominio_id,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updateData = {
        ...
        fecha_cierre: formData.estado === 'cerrado' ? new Date().toISOString() : null,
        tags: tags.length > 0 ? tags : undefined,
      }

      const { success, error } = await updateGestion(gestion.id, updateData)
      ...
      if (success) {
        ...
        router.push(`/gestiones/${gestion.id}`)
      } else {
        toast({
          title: "Error",
          description: error || "No se pudo actualizar la gestión",
          variant: "destructive",
        })
      }
    } catch (error) {
  ...
  return (
    <Card className="rounded-2xl">
      <CardHeader>
        <CardTitle>Editar Información de la Gestión</CardTitle>
        <CardDescription>
          Modifica los datos de la gestión y guarda los cambios
        </CardDescription>
      </CardHeader>
      <CardContent>
