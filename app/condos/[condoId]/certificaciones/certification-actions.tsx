...
export function CertificationActions({ certification, condoId }: CertificationActionsProps) {
  ...
  const handleDownload = async () => {
    if (!certification.cert_file_url) {
      toast({
        title: "Sin archivo",
    ...
    try {
      const result = await getSignedUrl(certification.cert_file_url)
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (certification.cert_file_url) {
        await deleteFile(certification.cert_file_url)
      }
  ...
  return (
    <div className="flex items-center gap-2">
      <Button variant="outline" size="sm" asChild className="rounded-lg bg-transparent">
        <Link href={`/condos/${condoId}/certificaciones/${certification.id}/edit`}>
