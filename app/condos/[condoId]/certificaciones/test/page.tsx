...
export default function TestCertificationPage({ params }: { params: Promise<{ condoId: string }> }) {
  ...
  const testConnection = async () => {
    ...
    try {
      ...
      // Prueba 3: Verificar permisos de lectura en certifications
      ...
      if (certError) {
        throw new Error(`Error al leer certificaciones: ${certError.message}`)
      }

      setResult(prev => prev + `✅ Permisos de lectura OK (${certData.length} certificaciones encontradas)\n`)

      // Prueba 4: Intentar insertar una certificación de prueba
      console.log('Prueba 4: Intentando insertar certificación de prueba...')
      const testData = {
        condo_id: condoId,
      ...
      // Prueba 5: Eliminar la certificación de prueba
      console.log('Prueba 5: Eliminando certificación de prueba...')
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold">Prueba de Certificaciones</h2>
        <p className="text-muted-foreground">Diagnóstico de problemas con la creación de certificaciones</p>
      </div>
      ...
      <Card className="rounded-2xl">
        <CardHeader>
          ...
          <CardDescription>
            Este formulario ejecuta una serie de pruebas para identificar problemas con la creación de certificaciones.
